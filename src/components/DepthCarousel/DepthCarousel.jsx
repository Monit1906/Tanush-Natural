...
const DepthCarousel = ({
  ...
  const onCardClick = useCallback(
    index => {
      ...
      // If clicking the currently active card, navigate to its link
      if (active === index && data[index]?.to) {
        navigate(data[index].to);
      } else {
        // Otherwise bring it to focus
        setFocus(index, true);
      }
    },
    [setFocus, data, active, navigate]
  );
      ...
      <div className="depth-carousel__stage" ref={stageRef}>
        {data.map((item, i) => (
          ...
          >
            ...
            {/* Added overlay title for category name */}
            <div className="depth-carousel__title-overlay">
              <h3 className="depth-carousel__title">{item.alt}</h3>
            </div>
